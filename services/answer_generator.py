...
class AnswerGenerator:
    def __init__(self):
        # Load enhanced knowledge bases
        self.enhanced_course_content = self.load_course_markdown_from_json()
        self.enhanced_discourse_posts = self.load_discourse_posts_from_json()
        self.comprehensive_knowledge = self.load_comprehensive_knowledge()
        ...
        self.predefined_answers = {
            'course_info': {
                'what_is_tds': {
                    'answer': "TDS (Tools in Data Science) is a practical diploma level course at IIT Madras covering 7 modules. It teaches real-world tools for data sourcing, analysis, and deployment.",
                    'links': [{'url': "https://tds.s-anand.net", 'title': "TDS Course Materials"}]
                },
                'tds_full_form': {
                    'answer': "TDS stands for 'Tools in Data Science'. It's a diploma-level course by IIT Madras that focuses on practical tools for handling and analyzing data.",
                    'links': [{'url': "https://tds.s-anand.net/#/2025-01", 'title': "TDS Course Overview"}]
                },
                'course_books': {
                    'answer': "There are no IITM certified books for TDS. The official reference is https://tds.s-anand.net.",
                    'links': [{'url': "https://tds.s-anand.net", 'title': "Official TDS Reference"}]
                }
            }
        }

    def load_course_markdown_from_json(self) -> List[Dict[str, Any]]:
        """Loads markdown files from 3_all_course_data.json as content blocks"""
        try:
            ...
            print(f"Error loading course markdown: {e}")
        return []

    def load_discourse_posts_from_json(self) -> List[Dict[str, Any]]:
        """Load posts from 4_discourse_posts_2025.json"""
        try:
            filepath = os.path.join('data', '4_discourse_posts_2025.json')
            if os.path.exists(filepath):
                with open(filepath, 'r', encoding='utf-8') as f:
                    raw_data = json.load(f)
                    structured = []
                    for thread in raw_data:
                        content = " ".join([post.get("content_html", "") for post in thread.get("posts", [])])
                        structured.append({
                            'title': thread.get("title", "Discourse Thread"),
                            'url': f"https://discourse.onlinedegree.iitm.ac.in/t/{thread.get('id', '')}",
                            'answer_summary': content[:300] + "..." if len(content) > 300 else content,
                            'keywords': []
                        })
                    return structured
        except Exception as e:
    ...
    def generate_answer(self, processed_question: Dict[str, Any]) -> Dict[str, Any]:
        question_type = processed_question.get('question_type', '')
        keywords = processed_question.get('keywords', [])
        original_question = processed_question.get('original_question', '')

        predefined = self.get_predefined_answer(question_type, keywords, original_question)
        if predefined:
            return predefined

        content_matches = self.search_enhanced_content(processed_question)
        if content_matches:
            return self.generate_contextual_answer(processed_question, content_matches)
        return self.generate_fallback_answer()

    def get_predefined_answer(self, question_type: str, keywords: List[str], question: str) -> Optional[Dict[str, Any]]:
        q_lower = question.lower()
        if any(k in q_lower for k in ['what is tds', 'tds course', 'tds overview']):
            return self.predefined_answers['course_info']['what_is_tds']
        if any(k in q_lower for k in ['tds full form', 'what does tds']):
            return self.predefined_answers['course_info']['tds_full_form']
        if any(k in q_lower for k in ['books', 'pdf', 'reference']):
            return self.predefined_answers['course_info']['course_books']
        return None

    def search_enhanced_content(self, processed_question: Dict[str, Any]) -> List[Dict[str, Any]]:
        q = processed_question.get('cleaned_question', '').lower()
        keywords = processed_question.get('keywords', [])
        results = []

        # Search course content
        for item in self.enhanced_course_content:
            score = 0
            ...
            for term in q.split():
                if len(term) > 3 and term in text:
                    score += 1
            if score > 0:
                results.append({'type': 'course_content', 'data': item, 'relevance': score})

        # Search discourse
        for item in self.enhanced_discourse_posts:
            score = 0
            text = (item.get("answer_summary", "") + " " + item.get("title", "")).lower()
            for k in keywords:
                if k.lower() in text:
                    score += 2
            for term in q.split():
                if len(term) > 3 and term in text:
                    score += 1
            if score > 0:
                results.append({'type': 'discourse', 'data': item, 'relevance': score})

        return sorted(results, key=lambda x: x['relevance'], reverse=True)[:3]

    def generate_contextual_answer(self, processed_question: Dict[str, Any], matches: List[Dict[str, Any]]) -> Dict[str, Any]:
        answer_parts = []
        links = []

        for m in matches:
            if m['type'] == 'course_content':
                data = m['data']
                answer_parts.append(data.get("content", ""))
                links.append({'url': data.get("url", ""), 'title': data.get("title", "Course Content")})
            elif m['type'] == 'discourse':
                data = m['data']
                answer_parts.append(data.get("answer_summary", ""))
                links.append({'url': data.get("url", ""), 'title': data.get("title", "Discourse Post")})
    ...
    def generate_fallback_answer(self) -> Dict[str, Any]:
        return {
            'answer': "I couldn't find a specific answer. Please refer to the official TDS resources linked below.",
            'links': [
                {'url': 'https://tds.s-anand.net', 'title': 'TDS Course Materials'},
                {'url': 'https://discourse.onlinedegree.iitm.ac.in', 'title': 'Discourse Forum'}
            ]
        }
